...
<body class="min-h-screen">
    ...
    <div class="min-h-[500px] max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-[50px] mt-[30px]">
            <h1 class="font-bold text-4xl">Edit</h1>
        </div>
        <div class="flex gap-8">
            <form method="post" action="{{ route('todo.update', ['todo' => $todo]) }}" class="flex flex-col gap-6 w-1/2">
                @csrf
                @method('put')
                <div class="flex flex-col relative">
                    <label for="name">Title</label>
                    <input type="text" name="name" placeholder="Enter title" value="{{ $todo->name }}"
                ...
                <div class="flex flex-col relative">
                    ...
                    <textarea name="description" id="description" cols="30" rows="5" style="resize: none"
                        placeholder="Enter description" class="@error('description') outline outline-1 outline-red-600 @enderror">{{ $todo->description }}</textarea>
                    @error('description')
                        <div class="text-[14px] text-red-600 absolute -bottom-5">{{ $message }}</div>
                    @enderror
                </div>
                <div class="flex flex-col relative">
                    ...
                    <input type="date" name="date" id="date" value="{{ $todo->date }}"
                ...
                <button type="submit"
                    class="text-center uppercase leading-none bg-[#5c7cfa] text-white px-[15px] py-[10px] shadow-md hover:bg-[#5c7cfa20] transition-all duration-500">Update</button>
            </form>
        </div>
    </div>
</body>
